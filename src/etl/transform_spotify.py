...
# Genre mapping for consolidation
CONSOLIDATED_GENRES = {
...
NON_SOUND_BASED_GENRES = [
    'british', 'brazilian', 'french', 'german', 'iranian', 'swedish', 'spanish', 'indian', 'malay', 'turkish', 'world-music', 'gospel'
]
...
def resolve_track_genre(df: pd.DataFrame, id_col: str = 'track_id') -> pd.DataFrame:
    ...
    df = df.merge(resolved, left_on=id_col, right_on=id_col)
    df['track_genre'] = df['resolved_genre']
    df = df.drop(columns=['resolved_genre'])
    return df
...
def transform_spotify_dataset(df: pd.DataFrame) -> pd.DataFrame:
    df = clean_spotify_data(df)
    df = consolidate_genres(df)
    df = drop_track_id_duplicates(df)
    df = transform_spotify_features(df)
    logging.info(f"Spotify dataset transformed with shape: {df.shape}")
    return df
